...
//
float inferenz_multiple_refinement(std::vector<Model> models,
    const std::vector<cv::Mat_<float> >& apperance_scores,
    const std::vector<cv::Point_<int> >& min_locations,
    std::vector<cv::Point_<int> >& refined_locations,
    std::vector<float>& mean_lengths,
    body_pose::BodyPoseTypes pose_type,
    std::vector<int> parents = std::vector<int>(),
    bool debug = false,
    cv::Mat img = cv::Mat(), float weight = 0.0, int num_threads = 0);

float inferenz_multiple_remove_duplicates(std::vector<Model> models,
    const std::vector<cv::Mat_<float> >& apperance_scores,
    const std::vector<std::vector<cv::Point_<int> > >& min_locations,
    std::vector<std::vector<cv::Point_<int> > >& refined_locations,
    std::vector<float>& mean_lengths,
    body_pose::BodyPoseTypes pose_type,
    float thresh = 5,
    bool debug = false,
