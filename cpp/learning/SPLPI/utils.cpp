...
void nms(vector<learning::splpi::Detection>& src, vector<learning::splpi::Detection>& dst, float thresh)
{
  if(!src.size()) return;
...
void group_detections(vector<learning::splpi::Detection>& src, vector<learning::splpi::Detection>& dst, float thresh)
{
  if(!src.size()) return;
  dst.clear();

  std::vector<int> I(src.size());
  std::iota(I.begin(), I.end(), 0); // Fill with 0, 1, ..., 99.

  while(I.size()){
    int last = I.size()-1;
    int i = I[last];

    learning::splpi::Detection d = src[i];
    Point pt1 = d.loc;
    ...
    for(int pos=0; pos<last; pos++){
      ...
      float dist = sqrt(pow(pt1.x-pt2.x, 2) + pow(pt1.y - pt2.y, 2));
      if(dist <= thresh){
        d.possible_labels.push_back(src[j].label);
        src[j].possible_labels.push_back(d.label);
    ...
    if(d.possible_labels.size() > 1){
      sort(d.possible_labels.begin(), d.possible_labels.end());
      vector<int>::iterator it;
      it = unique(d.possible_labels.begin(), d.possible_labels.end());
      d.possible_labels.resize(distance(d.possible_labels.begin(),it));
    }
...
//      LOG(INFO)<<i<<"\t"<<::utils::VectorToString(dst[i].possible_labels);
//  }
//  LOG(INFO)<<"/////////////////";
...
void draw_detections(cv::Mat image, std::vector<learning::splpi::Detection>& detections, bool draw_lines)
{
  ...
  for(unsigned int dIdx=0; dIdx<detections.size(); dIdx++){
    ...
    if(detections[dIdx].label <= 9){
      circle(plot, detections[dIdx].loc, 5, joint_colors[label], 2);
    }else{
      circle(plot, detections[dIdx].loc, 5, joint_colors[label], -1);
    }
