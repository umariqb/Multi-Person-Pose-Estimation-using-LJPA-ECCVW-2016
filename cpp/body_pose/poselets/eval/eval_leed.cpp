...
DEFINE_string(forest_path, "/scratch_net/giggo/mdantone/grid/poselets_leed/single/single_1/pixel_hard_neg_reg", "folder wehere all the forests are stored");
...
DEFINE_string(augmented_forest_path, "", "folder wehere all the forests are stored");
...
int main(int argc, char** argv) {
  ...
  const boost::filesystem::path augmented_forest_path  = FLAGS_augmented_forest_path;
  ...
  if(experiment_name != "") {
    ...
    string base = "/scratch_net/giggo/mdantone/grid/poselets_leed/";
    ...
    if(false && forests.size() == 13) {

      vector<learning::forest::Forest<body_pose::poselets::PoseletSample> >  forests_ = forests;
      vector<body_pose::poselets::Poselet> poselets_ = poselets;
      forests.resize(13);
      poselets.resize(13);
      for(int i=0; i<13; i++) {
        int index = poselets_[i].part_indices[0];
        poselets[index] = poselets_[i];
        forests[index] = forests_[i];
      }
    }
  }
  ...
  if(augmented_forest_path != ""){
    fs::path tree_save_dir = augmented_forest_path.string()+"/forest_1";
    fs::path forest_config_file = augmented_forest_path.string()+"/config.txt";
    LOG(INFO) << augmented_forest_path;
...
//    vision::image_utils::extend_image(image_org, 20, image);
    ...
    if(augmented_forest_path != "") {
      sample.init(image, augment_forest_param.features, &fcf, false, i);
      eval_single_image(&augment_forest, &sample);

      if(debug_mode) {
        sample.display_feature_channels();
      }
    }else{
    ...
    eval_voting_poselets_mt(forests, poselets, sample, &voting_maps, 1, mode);

//    Rect roi(0,0, sample.width(), sample.height());
//    voting_maps.resize(13);
//    get_multiple_forground_map_mt(sample, forests, voting_maps, roi, 1);
