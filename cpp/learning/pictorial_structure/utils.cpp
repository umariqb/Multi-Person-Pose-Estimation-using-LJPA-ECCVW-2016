...
bool get_displacement_cost(
  ...
  std::vector< std::vector<cv::Point> > annotations = annotations_org;
  ...
  int num_parts = get_num_parts(pose_type);

  /// i_part and part_id are separated to allow fully connected graph models.
  /// For simple graph structures (tree models) nothing will change.
  for(int i_part =0; i_part < parents.size(); i_part++) {
    int part_id = i_part % num_parts;
...
//    std::cout<<"Part-id = "<<i_part<<std::endl;
    ...
    for(int i=0; i < annotations.size(); i++) {
      ...
      const cv::Point& part = annotations[i][part_id];
...
//      CHECK_LT(offset.x, 260);
//      CHECK_LT(offset.y, 260);

      offsets_per_part.push_back(offset);
    }
//    LOG(INFO)<<offsets_per_part.size();
//    std::cout<<i_part<<std::endl<<std::endl;
//    utils::visualize_offsets(offsets_per_part);


    Displacement displacement;
    if(part_id != parents[i_part]) {

      if(params[part_id].joint_type == ROT_GAUSS) {
        cv::Point_<float> mean_offset;
        float mean_l = 0;
        ...
        int num_rotations = params[part_id].num_rotations;
        ...
        for (int i=0; i < orientation_hist.size(); i++){
          if( (orientation_hist[i]) > 0 || (params[part_id].use_weights == false) ) {
...
//        LOG(INFO) << i_part << ") sifts: "<<  displacement.shifts.size() ;
        ...
        if(false) {
          ...
          visualize_clusters(offsets_per_part, centers, best_labels, part_id );
        }

      }else if(params[part_id].joint_type == CLUSTER_GAUSS){

        int n_clusters = params[part_id].num_rotations;

        cv::Mat best_labels;
        cv::Mat centers;

        if(!params[part_id].use_ann_weights){
          cv::Mat features(cv::Size(2,offsets_per_part.size()),CV_32FC1);
          if(offsets_per_part.size() < n_clusters){
            return false;
          }
          for(int i=0; i < offsets_per_part.size(); i++) {
...
//        visualize_clusters(offsets_per_part, centers, best_labels, i_part );
        ...
        for(int i=0; i < n_clusters; i++) {
          ...
          if(offsets.size() >  params[part_id].min_sampels ) {
            ...
            if(params[part_id].use_weights) {
              float prob = offsets.size() / static_cast<float> (annotations.size());
              float w = -log(prob) * params[part_id].weight_alpha;
...
//    LOG(INFO)<<i_part<<": "<<displacements[i_part].shifts.size();
