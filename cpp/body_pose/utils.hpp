...
void get_mask(int part_id, const cv::Mat& img, const Annotation ann, cv::Mat_<uchar>& mask, body_pose::BodyPoseTypes pose_type, float size = 0, bool debug = false);
...
float get_avg_group_scale(MultiAnnotation& ann, float scale_factor = 1);

float rescale_img_with_avg_scale(const cv::Mat& src,
              cv::Mat& dest, MultiAnnotation& ann,
              float scale_factor = 1);

float rescale_img_with_avg_group_scale(const cv::Mat& src,
              cv::Mat& dest, MultiAnnotation& ann,
              float scale_factor = 1);
...
void extract_group_roi_mpii( const cv::Mat& img, MultiAnnotation& ann,
                  cv::Mat& img_roi , int offset_x,
                  int offset_y, cv::Rect* extracted_region);
...
void eval_forests_cnn(const std::vector<learning::forest::Forest<BodyPartSample> >& forests,
      const learning::common::Image& sample,
      std::vector<cv::Mat_<float> >& voting_maps,
      int stepsize = 1, bool regression = false,
      int num_threads = 0,
...
void eval_attr_wise_forests_cnn(const std::vector<learning::forest::Forest<BodyPartSample> >& forests,
      const learning::common::Image& sample,
      std::vector<std::vector<cv::Mat_<float> > >& voting_maps,
      int stepsize,
      bool regression,
...
bool load_forests(std::string base_url,
  ...
  std::vector<std::string> config_files;
  {
    ...
    if(pose_type == body_pose::FULL_BODY_J13){
      ...
      LOG(ERROR)<<"Body pose type is not supported or wrong!";
    }
  }
  ...
  for(int i=0; i < config_files.size(); i++) {
    ...
    forest.load(param_part.tree_path, param_part, max_trees);
...
template <typename T>
bool update_forests(std::string base_url,
    std::vector<learning::forest::Forest<T> >& forests,
    body_pose::BodyPoseTypes pose_type,
    int max_trees = -1,
    int max_depth = 20,
  ...
  CHECK_EQ(forests.size(), config_files.size());

  for(int i=0; i < config_files.size(); i++) {
    learning::forest::ForestParam param_part;
    CHECK(learning::forest::loadConfigFile(config_files[i], param_part));
    forests[i].load(param_part.tree_path, param_part, max_trees);

    if(param_part.max_depth > max_depth) {
      forests[i].prune_trees(max_depth);
    }
  }
  return forests.size() == pair_names.size();
}
...
void readMatAsciiWithoutHeader(std::string filename, cv::Mat& matData, int numRows, int numCols);

void remove_duplicates(std::vector<std::vector<cv::Point_<int> > >& in_parts,
              std::vector<std::vector<cv::Point_<int> > >& out_parts,
              float thresh);
